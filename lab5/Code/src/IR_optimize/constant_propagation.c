...
static CPValue meetValue(CPValue v1, CPValue v2) {
     ...
     */
    // TODO();
    if(v1.kind==UNDEF && v2.kind==CONST)
        return get_CONST(v2.const_val);
    else if(v2.kind==UNDEF&&v1.kind==CONST)
        return get_CONST(v1.const_val);
    else if(v1.kind==CONST && v2.kind==CONST && v1.const_val==v2.const_val)
        return get_CONST(v1.const_val);
    else if(v1.kind==UNDEF && v2.kind==UNDEF)
        return get_UNDEF();//不会走到这一步
    else 
        return get_NAC();
}
...
Fact_meet_value(Map_IR_var_CPValue *fact, IR_var var, CPValue val) {
    CPValue old_val = Fact_get_value_from_IR_var(fact, var); //获得旧值
    CPValue new_val = meetValue(old_val, val);
    if(old_val.kind == new_val.kind && old_val.const_val == new_val.const_val)//没变
        return false;
    Fact_update_value(fact, var, new_val);//变了，要更新
    return true;
}
...
void ConstantPropagation_transferStmt (ConstantPropagation *t,
                                       IR_stmt *stmt,
                                       Map_IR_var_CPValue *fact) {
    if(stmt->stmt_type == IR_ASSIGN_STMT) {//赋值语句
        ...
        if(def != IR_VAR_NONE) {
             ...
             */
            // TODO();
            Fact_update_value(fact, def, get_UNDEF()); //这个怎么填哇
        }
    }
}
...
static void block_constant_folding (ConstantPropagation *t, IR_block *blk) {
    Map_IR_var_CPValue *blk_in_fact = VCALL(*t, getInFact, blk);  //获得当前块的blk
    Map_IR_var_CPValue *new_in_fact = ConstantPropagation_newInitialFact(t); //新建一个
    ConstantPropagation_meetInto(t, blk_in_fact, new_in_fact);//用blk去更新new
    for_list(IR_stmt_ptr, i, blk->stmts) {//遍历每个语句
        IR_stmt *stmt = i->val;
        IR_use use = VCALL(*stmt, get_use_vec); //获得use
        for(int j = 0; j < use.use_cnt; j++) 
            if(!use.use_vec[j].is_const) { //不是常量
                IR_var use_var = use.use_vec[j].var; 
                CPValue use_CPVal = Fact_get_value_from_IR_var(new_in_fact, use_var);//查表，查不到就是undef
                if(use_CPVal.kind == CONST)  //查到是CONST
                    use.use_vec[j] = (IR_val){.is_const = true, .const_val = use_CPVal.const_val}; //替换为const
            }
        ConstantPropagation_transferStmt(t, stmt, new_in_fact);//遍历完该语句所有的use
    }
    RDELETE(Map_IR_var_CPValue, new_in_fact);
}

void ConstantPropagation_constant_folding (ConstantPropagation *t, IR_function *func) {
    for_list(IR_block_ptr, j, func->blocks) {//遍历每个基本块，应用常量折叠
        IR_block *blk = j->val;
