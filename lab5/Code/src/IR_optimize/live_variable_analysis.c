...
LiveVariableAnalysis_meetInto (LiveVariableAnalysis *t,
     ...
     */
    // TODO();
    return VCALL(*target, union_with, fact); //并集
}

void LiveVariableAnalysis_transferStmt (LiveVariableAnalysis *t,
                                        IR_stmt *stmt,
                                        Set_IR_var *fact) {
    IR_var def = VCALL(*stmt, get_def);//获得def
...
static bool block_remove_dead_def (LiveVariableAnalysis *t, IR_block *blk) { //死代码消除，消除某个块的（局部）
    bool updated = false;
