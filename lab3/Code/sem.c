...
// #define DEBUG_struct
    ...
    #define DEBUG_hash
#endif
void printArra(const Var v){
    assert(v->type->kind==ARRAY);
    Type tmp  = v->type;
    printf("#####print ARRAY begin####\n");
    printf("ARRA name: %s, typeSize: %d\n",v->name, typeSize(tmp));
    while(tmp->u.array.elem->kind==ARRAY){
        printf("typeSize: %d, dimension: %d\n", typeSize(tmp->u.array.elem), tmp->u.array.size);
...
void printStruct(const FieldList struc){
    printf("#######print struct begin#######\n");
    FieldList fl = struc;
    assert(fl);
    while(fl){
        printf("  field name: %s, type: %d\n",fl->name, fl->type->kind);
        if(fl->type->kind == ARRAY){
            printf("        arra elem: type: %d, size: %d\n", fl->type->u.array.elem->kind, fl->type->u.array.size);
        }
        fl = fl->tail;
    }
    printf("#######print struct end#######\n");
}
...
void semAnaly(pNode root){
    ...
    char* n = (char*)malloc(sizeof(char)*10);//write函数的参数
    strcpy(n, "a");
...
Type Specifier(pNode root){//finish
    ...
    if(!strcmp(root->child[0]->token,"TYPE")){
        ...
        // printf("t->kind: %d, name: %s, type: %d\n",t->kind, t->u.structure->name, t->u.structure->type->kind);
        // printStruct(t->u.structure);
        #ifdef DEBUG
...
void VarList(pNode root, const Var  fun){//finish
    ...
    // }
    if(root->childNum==3)
        VarList(root->child[2],fun);//获得下一个参数
    ...
    fl->type = v->type;
    fl->name = v->name;
    fl->tail = fun->type->u.func.args;//把这个参数加到函数参数列表的头部
    fun->type->u.func.args = fl;
}
